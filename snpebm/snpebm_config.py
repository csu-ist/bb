...
class AccuracyDnnModel(DnnModel):
    def __init__(self, config, host_artifacts):
        DnnModel.__init__(self, config, host_artifacts)
    ...
    def __quick_verify__(self):
        try:
            ...
            for _key in CONFIG_JSON_ROOTKEYS:
                # check optional keys
                if not _key is CONFIG_PERF_PROFILE_KEY and not _key is CONFIG_CPU_FALLBACK_KEY and not _key is CONFIG_HOST_NAME_KEY:
...
class AccuracyConfig(Config):
    def __init__(self, cfg, outputbasedir, devicelist, hostname, deviceostype, logger, userbuffer_mode, perfprofile):
        Config.__init__(self, cfg, outputbasedir, devicelist, hostname, deviceostype, logger, userbuffer_mode, perfprofile)
        self._dnnmodel = AccuracyDnnModel(self._cfg_from_json, self.host_artifacts)
...
class ConfigFactory:
    ...
    def make_config(cfgfile, outputbasedir, devicelist, hostname, usedevicesconnected, deviceostype, logger, userbuffer_mode, perfprofile):
        ...
        if MEASURE_ACCURACY not in _config_from_json[CONFIG_MEASUREMENTS_KEY]:
            ...
            return AccuracyConfig(_config_from_json, outputbasedir, devicelist, hostname, deviceostype, logger, userbuffer_mode, perfprofile)
        else:
